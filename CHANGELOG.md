...
- **The shipped lock is a Linux resolve again, and names no sibling
  checkout.** Pre-publish hardening, `B30-pre-publish-hardening` leg 2
  (finding `G24`). `worker/requirements_prod.lock` still pinned
  **`colorama==0.4.6`** with no environment marker (a Windows-only shim under
  `click`, left by a Windows regeneration) and carried an active
  `--find-links ../../media-sdk-m8/dist` option, a path into a parent
  checkout inside a release build input. The lock was regenerated inside the
  Dockerfile's own pinned `python:3.14-slim` digest, resolving from PyPI only
  (`media-sdk-m8==1.0.0` resolves there with the hashes already recorded),
  with no `--upgrade` of any kind. Exactly two things leave — the
  `--find-links` line and the `colorama` block — and the autogenerated
  header comment loses the flag. Nothing else moves. Read out of the built
  images:

  | Distribution | `main` before this fix | This release |
  | --- | --- | --- |
  | `colorama` | `0.4.6` | **absent** |
  | every other distribution (31) | unchanged | unchanged |

### Security

- **A lock that only resolves where it was made is now refused.**
  `scripts/shipped_lock_env.py --check-portable` runs first in
  `test-shipped-lock` and fails on a `--find-links`/`-f`/`--index-url`/
  ...
  `pywin32-ctypes`, `pywinpty`). It read the previous lock as not portable
  (both lines above) before the regeneration. Until now nothing asserted the
  platform a lock was generated on, so the next Windows regeneration would
  have put `colorama` back silently. The same check is in all five service
  repositories. `tests/test_shipped_lock_portability.py` pins its behaviour
  and its place in the job.
- **The arm64 image is scanned before it is pushed.** `docker-publish.yaml`
  scanned a `linux/amd64` build and then pushed a separate
  `linux/amd64,linux/arm64` build, so the arm64 image reached the registry
  unscanned (finding `G31`(a), leg 8). A second single-platform `load: true`
  build under the QEMU the workflow already installs is now Trivy-gated at
  the same settings before the push. `tests/test_publish_scans_every_platform.py`
  fails if any pushed platform lacks its own blocking scan. Both
  architectures of this release read **0** CRITICAL/HIGH locally.
